...
The quickest way to retrieve your `chat_id` is visiting `https://api.telegram.org/bot<API-access-token>/getUpdates?offset=0` or to use `$ curl -X GET https://api.telegram.org/bot<API-access-token>/getUpdates?offset=0` **after** you have sent the bot a message. Replace `YOUR_API_TOKEN` with your actual token.
